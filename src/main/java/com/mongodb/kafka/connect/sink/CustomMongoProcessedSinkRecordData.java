...
import com.mongodb.kafka.connect.sink.writemodel.strategy.CustomWriteModelStrategyHelper;

final class CustomMongoProcessedSinkRecordData {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(CustomMongoProcessedSinkRecordData.class);
  ...
  private final List<WriteModel<BsonDocument>> writeModel;
  private Exception exception;

  CustomMongoProcessedSinkRecordData(
      final SinkRecord sinkRecord, final MongoSinkConfig sinkConfig) {
  ...
  public List<WriteModel<BsonDocument>> getWriteModel() {
    return writeModel;
  }
  ...
  private List<WriteModel<BsonDocument>> createWriteModel() {
    return buildWriteModel();
  }

  private List<WriteModel<BsonDocument>> buildWriteModel() {
    return tryProcess(
            () -> {
              config
                  .getPostProcessors()
              ...
              return Optional.ofNullable(
                  CustomWriteModelStrategyHelper.createWriteModel(config, sinkDocument));
            })
        .orElse(null);
  }
