...
final class CustomStartedMongoSinkTask implements AutoCloseable {
  ...
  void put(final Collection<SinkRecord> records) {
    try (InnerTimer automatic = inTaskPutInConnectFrameworkTimer.sampleOuter()) {
      ...
      if (records.isEmpty()) {
        ...
        List<List<CustomMongoProcessedSinkRecordData>> batches =
            CustomMongoSinkRecordProcessor.orderedGroupByTopicAndNamespace(
                records, sinkConfig, errorReporter);
        statistics.getProcessingPhases().sample(processingTime.getElapsedTime().toMillis());
        for (List<CustomMongoProcessedSinkRecordData> batch : batches) {
  ...
  private void bulkWriteBatch(final List<CustomMongoProcessedSinkRecordData> batch) {
    if (batch.isEmpty()) {
      return;
    }
    ...
    List<WriteModel<BsonDocument>> writeModels =
        batch.stream().flatMap(data -> data.getWriteModel().stream()).collect(Collectors.toList());
    ...
    try {
      ...
      handleTolerableWriteException(
          batch.stream()
              .map(CustomMongoProcessedSinkRecordData::getSinkRecord)
    ...
    if (e instanceof MongoBulkWriteException) {
      AnalyzedBatchFailedWithBulkWriteException analyzedBatch =
          new AnalyzedBatchFailedWithBulkWriteException(
              ...
              errorReporter,
              CustomStartedMongoSinkTask::log);
      if (logErrors) {
        LOGGER.error(
